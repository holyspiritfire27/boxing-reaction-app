...
# ==========================================
# 0. 自動下載中文字型 (解決亂碼與方塊問題)
# ==========================================
FONT_PATH = "NotoSansTC-Regular.ttf"
if not os.path.exists(FONT_PATH):
    try:
        # 從 Google Fonts Github 下載開源中文字型
        url = "https://github.com/google/fonts/raw/main/ofl/notosanstc/NotoSansTC-Regular.ttf"
        urllib.request.urlretrieve(url, FONT_PATH)
    except Exception as e:
        print("字型下載失敗", e)

# 載入字型 (若下載失敗則為 None，自動回退預設)
try:
...
PHYSICAL_SHOULDER_WIDTH_M = 0.35 # 參考肩寬 (m)
...
class BoxingPoseProcessor:
    ...
    def generate_summary_image(self):
        ...
        draw.text((W//2 - 180, 40), "🥊 拳擊訓練成果結算 🥊", fill=(0, 255, 0), font=font_huge)
        ...
        draw.text((box_l[0]+30, box_l[1]+90), f"平均拳速", fill=(200, 200, 200), font=font_medium)
        ...
        draw.text((box_r[0]+30, box_r[1]+90), f"平均拳速", fill=(200, 200, 200), font=font_medium)
        ...
        draw.text((W//2 - 120, H-50), f"訓練時間: {time.strftime('%Y-%m-%d %H:%M')}", fill=(150, 150, 150), font=font_medium)
        return image
    ...
    def recv(self, frame: av.VideoFrame) -> av.VideoFrame:
        ...
        if results.pose_landmarks:
            ...
            # 👉 A. 【完美修復】鏡像翻轉導致的左右手錯置問題！
            # 因為畫面鏡像，MediaPipe 的 RIGHT_WRIST 其實就是你真實的「左手」！
            lw_landmark = landmarks[mp_pose.PoseLandmark.RIGHT_WRIST.value] # 你真實的左手
            rw_landmark = landmarks[mp_pose.PoseLandmark.LEFT_WRIST.value]  # 你真實的右手
            
            lw = (int(lw_landmark.x * w), int(lw_landmark.y * h))
            rw = (int(rw_landmark.x * w), int(rw_landmark.y * h))
            
            # 計算瞬間拳速 (px/s 轉 m/s)
            if self.prev_lw and dt > 0:
                dist_l_px = np.sqrt((lw[0] - self.prev_lw[0])**2 + (lw[1] - self.prev_lw[1])**2)
                self.left_speed = (dist_l_px / dt) * self.meters_per_pixel
            if self.prev_rw and dt > 0:
                dist_r_px = np.sqrt((rw[0] - self.prev_rw[0])**2 + (rw[1] - self.prev_rw[1])**2)
                self.right_speed = (dist_r_px / dt) * self.meters_per_pixel
            ...
            if self.state == "WAITING":
                ...
                # 使用下載好的中文字型 (支援中文)
                draw.text((box_l + 15, box_t + 18), p_text, fill=(255, 255, 255), font=font_large)
        ...
        if self.state == "FINISHED":
            ...
            dashboard_text = (
                ...
                f"左手爆發: {self.left_speed:.2f} m/s\n"
                f"右手爆發: {self.right_speed:.2f} m/s"
            )
...
# ==========================================
def main():
    st.title("🥊 拳擊反應與測速訓練器 (完美中文版)")
    st.markdown(f"目標固定在**下巴**位置，手腕靠近整個**頭部範圍**即算擊中。完成 **{TOTAL_GAME_PUNCHES}** 拳後將自動結算結果。")
